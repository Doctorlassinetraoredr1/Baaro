    ...
    <div className="rounded-xl border p-3" style={{borderColor:COLORS.borderTeal,background:COLORS.surface}}>
      ...
      <div className="space-y-2">
        {rows.map((row) => {
          ...
          const p=pct(Number(row.vote_count||0),total);
          return (
            <button key={row.option_id} type="button" disabled={busy} onClick={()=>vote(row.option_id)}
              ...
              <span className="relative flex items-center justify-between gap-3">
                <span>{row.option_text}{active?" ✓":""}</span><span className="font-bold">{p}% · {row.vote_count}</span>
              </span>
            </button>
          );
        })}
      </div>
      <p className="text-[10px] mt-2" style={{color:COLORS.muted}}>{total} vote{total>1?"s":""} · Vous pouvez changer votre vote</p>
    </div>
  );
}
...
export function SocialPostEnhancements({ post, userId }) {
  ...
  const load=useCallback(async()=>{
    ...
    const [r,s,b,f]=await Promise.all([
      ...
      userId ? supabase.from("post_bookmarks").select("id").eq("post_id",post.id).eq("user_id",userId).maybeSingle() : Promise.resolve({data:null}),
    ...
    setReaction(reactionRows.find(x=>x.user_id===userId)?.reaction||null);
  ...
  const chooseReaction=async(value)=>{
    ...
    try{
      if(reaction===value){
        const {error}=await supabase.from("post_reactions").delete().eq("post_id",post.id).eq("user_id",userId);
        if(error) throw error; setReaction(null); setReactionCount(n=>Math.max(0,n-1));
      }else{
        const {error}=await supabase.from("post_reactions").upsert({post_id:post.id,user_id:userId,reaction:value},{onConflict:"post_id,user_id"});
  ...
  const bookmark=async()=>{
    ...
    try{
      if(saved){
        const {error}=await supabase.from("post_bookmarks").delete().eq("post_id",post.id).eq("user_id",userId);
        if(error) throw error; setSaved(false);
      }else{
        const {error}=await supabase.from("post_bookmarks").insert({post_id:post.id,user_id:userId});
  ...
  const share=async()=>{
    ...
    try{
      ...
      const {error}=await supabase.from("post_shares").insert({post_id:post.id,user_id:userId||null,channel});
      if(error) throw error; setShareCount(n=>n+1);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {post.author_id!==userId && (
        <button type="button" onClick={follow} className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[11px] font-bold border"
          style={{borderColor:following?COLORS.borderTeal:COLORS.border,color:following?COLORS.teal:COLORS.muted}}>
          {following?<Check size={13}/>:<UserPlus size={13}/>} {following?"Abonné":"Suivre"}
        </button>
      )}
      <div className="relative">
        <button type="button" onClick={()=>setPicker(v=>!v)} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs border"
