...
export function SocialPostEnhancements({ post, userId }) {
  ...
  const share = async () => {
    ...
    try {
      ...
      if (error?.name !== "AbortError") showToast("Partage non enregistré", "error");
    }
  };

  const follow = async () => {
    if (!userId) return showToast("Connectez-vous pour suivre", "info");
    if (userId === post.author_id || busy) return; 
    setBusy(true);
    try {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="relative">
        ...
        {picker && (
          <div className="absolute bottom-full left-0 z-30 mb-1 flex gap-1 rounded-xl border p-2 shadow-xl" style={{ background: COLORS.surface, borderColor: COLORS.borderGold }}>
            {REACTIONS.map(x => (
              <button key={x.id} type="button" title={x.title} onClick={() => chooseReaction(x.id)} className="rounded-lg p-1.5 text-lg hover:bg-white/10">
                {x.label}
              </button>
            ))}
          </div>
