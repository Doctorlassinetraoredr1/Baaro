...
export const FriendsTab = ({ onOpenProfile }) => {
  const { id, isGuest } = useApp();
  const { friends, fetchFriends, loading, error } = useSocial(id);

  useEffect(() => {
    console.log("🔍 DEBUG FriendsTab - ID:", id, "| isGuest:", isGuest);
    if (id && !isGuest) {
      fetchFriends();
    }
  }, [id, isGuest, fetchFriends]);
  ...
  if (error) {
    return (
      <div className="p-4 text-center" style={{ color: '#ff4444' }}>
        Erreur : {error}
      </div>
    );
  }
